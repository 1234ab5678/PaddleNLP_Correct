...
class HtmlToDocx(HTMLParser):
    ...
    def handle_table(self, html, doc):
        ...
        # print(table_soup)
        # table_soup = BeautifulSoup(html, 'html5lib')
        ...
        for index, row in enumerate(rows):
            ...
            for col in cols:
                ...
                # print(cell_html) # 该语句输出的是每个cell的内容
                if col.name == 'th':
                    cell_html = "<b>%s</b>" % cell_html # 该语句实现字体加粗
                # print('搜索初始坐标')
                # print(cell_row)
                # print(cell_col)
                ...
                # print(index)
                while True:
                    # print((cell_row, cell_col) in points)
                    # print(not (index == 0))
                    if (((cell_row, cell_col) in points) and (not (index == 0))):
                        # print('有必要更换初始坐标')
                        cell_col = cell_col + 1
                    else:
                        break
                points.append((cell_row, cell_col))
                # print(points)
                # print('搜索新的坐标')
                # print(cell_row)
                # print(cell_col)
                docx_cell = table.cell(cell_row, cell_col)
                # print(type(docx_cell))
                if (cell_col + colspan -1) >= cols_len: # 合并单元格时防止列数越界
                    colspan -= 1

                # print(cell_row + rowspan - 1)
                # print(cell_col + colspan - 1)
                cell_to_merge = table.cell(cell_row + rowspan - 1,
                                           cell_col + colspan - 1)
                for index_i in range(cell_row, cell_row + rowspan, 1):
                    for index_j in range(cell_col, cell_col + colspan, 1):
                        points.append((index_i, index_j))
                # points.append((cell_row + rowspan - 1, cell_col + colspan - 1))
                # print(points)
                if docx_cell != cell_to_merge:
                    docx_cell.merge(cell_to_merge) # 合并单元格，merge方法的意思是将docx_cell和cell_to_merge合并起来
